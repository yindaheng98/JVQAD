...
from jnerf.ops.code_ops.global_vars import proj_options
...
class DensityGridSampler(nn.Module):
    def __init__(self, update_den_freq=16, update_block_size=5000000):
        ...
        self.calc_rgb = CalcRgb(self.density_grad_host_header, self.aabb_range, self.n_rays_per_batch,
                                self.MAX_STEP, self.padded_output_width, self.background_color, using_fp16=self.using_fp16)
    ...
    def sample(self, img_ids, rays_o, rays_d, rgb_target=None, is_training=False):
        ...
        if self.using_fp16:
            with jt.flag_scope(auto_mixed_precision_level=5):
                ...
                coords_compacted, rays_numsteps_compacted, compacted_numstep_counter = self.compacted_coords(
                    nerf_outputs, coords, rays_numsteps)
